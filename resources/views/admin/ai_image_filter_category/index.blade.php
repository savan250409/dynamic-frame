      ...
      <div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-center pb-3 mb-4" style="border-bottom:2px solid #f0f0f0;">
          ...
          <div class="d-flex align-items-center gap-2">
            ...
            <a href="{{ route('ai-image-filter-categories.create') }}" class="btn btn-primary btn-sm ms-1">
                  ...
                  <tr>
                    <td>
                      @if($category->category_image)
                        <img src="{{ asset('upload/ai_image_filter/'.rawurlencode($category->category_name).'/thumbnail/'.rawurlencode($category->category_image)) }}"
                    ...
                    <td class="text-end">
                      <a href="{{ route('ai-image-filter-categories.edit', $category->id) }}"
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title"><i class="fas fa-sort me-1"></i> Category Indexing</h5>
...
$(document).ready(function () {
  ...
  $(document).on('change', '.status-toggle', function () {
    ...
    $.ajax({
      url: "{{ route('ai-image-filter-categories.update-status') }}",
      type: 'POST',
  ...
  $(document).on('click', '.delete-btn', function () {
    ...
    Swal.fire({
      ...
      /* ── Step 2: final warning with full impact ── */
      Swal.fire({
        ...
        $.ajax({
          ...
          success: function (res) {
            if (res.success) {
              Swal.fire({
                title: 'Deleted!',
                text:  'Category "' + catName + '" and all its filters have been deleted.',
                icon:  'success',
                timer: 2500,
  ...
  function fetchData(page) {
    $.ajax({
      url: "{{ route('ai-image-filter-categories.index') }}",
  ...
  function saveOrder() {
    ...
    $.ajax({
      url: "{{ route('ai-image-filter-categories.update-order') }}", type: 'POST',
  ...
  $('#open-index-modal').on('click', function () {
    ...
    $('#index-loader').show();
    $.ajax({
      url: "{{ route('ai-image-filter-categories.order-list') }}",
      success: function (res) {
        $('#index-loader').hide();
